...
class RoboticAssistant:
    ...
    def render(self, img=np.ones((600, 600, 3))):
        ...
        color = (0, 0, 0)
        size = 1
        cv2.line(img, tuple(ed1.astype(np.int64).tolist()), tuple(ed2.astype(np.int64).tolist()), color, size)
        cv2.line(img, tuple(ed1.astype(np.int64).tolist()), tuple(ed3.astype(np.int64).tolist()), color, size)
        cv2.line(img, tuple(ed3.astype(np.int64).tolist()), tuple(ed4.astype(np.int64).tolist()), color, size)
        cv2.line(img, tuple(ed2.astype(np.int64).tolist()), tuple(ed4.astype(np.int64).tolist()), color, size)
        ...
        img = cv2.line(img, tuple(w1.astype(np.int64).tolist()), tuple(w2.astype(np.int64).tolist()), (0, 0, 0), 1)
        return img
...
def view_unit(rendered_unit, x, y, u, v, angle, color=(0, 0, 0), size=1):
    ...
    cv2.line(rendered_unit, tuple(edge1.astype(np.int64).tolist()), tuple(edge2.astype(np.int64).tolist()), color, size)
    cv2.line(rendered_unit, tuple(edge1.astype(np.int64).tolist()), tuple(edge3.astype(np.int64).tolist()), color, size)
    cv2.line(rendered_unit, tuple(edge3.astype(np.int64).tolist()), tuple(edge4.astype(np.int64).tolist()), color, size)
    cv2.line(rendered_unit, tuple(edge2.astype(np.int64).tolist()), tuple(edge4.astype(np.int64).tolist()), color, size)
