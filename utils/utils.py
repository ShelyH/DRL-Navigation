...
def distance_to_obstacle(robot_position, robot_sight_param, sensor_data):
    ...
    for i in range(robot_sight_param[0]):
        ...
        # print([robot_position[0] + sensor_data[i] * np.cos(np.deg2rad(theta)),
        #                         robot_position[1] + sensor_data[i] * np.sin(np.deg2rad(theta))])
        depth_awareness.append([robot_position[0] + sensor_data[i] * np.cos(np.deg2rad(theta)),
                                robot_position[1] + sensor_data[i] * np.sin(np.deg2rad(theta))])
